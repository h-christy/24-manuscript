...
adoption using approaches from spatial data science.
...
\tightlist
\item
  Geometries used in this project are from Census Bureau's TIGER/Line
  Shapefiles

  \begin{itemize}
  \tightlist
  \item
    State Boundaries(\citeproc{ref-Tl_2020_53_state20}{{``Index of
    /Geo/Tiger/{TIGER2020PL}/{STATE}/53\_{WASHINGTON}/53''} n.d.a})
  \item
    Census Tracts(\citeproc{ref-Tl_2020_53_tract20}{{``Index of
    /Geo/Tiger/{TIGER2020PL}/{STATE}/53\_{WASHINGTON}/53''} n.d.b})
  \item
    Zip Codes(\citeproc{ref-Tl_2022_us_zcta520}{{``Index of
    /Geo/Tiger/{TIGER2022}/{ZCTA520}''} n.d.})
  \item
    Urban Areas(\citeproc{ref-Tl_2020_us_uac20}{{``Index of
    /Geo/Tiger/{TIGER2020}/{UAC}''} n.d.})
  \end{itemize}
\end{itemize}
...
overlaps.\footnote{Seattle-Tacoma-Bellevue
  ...
  areas by the Census Bureau, ranking 15th, 25th and 96th respectively}(\citeproc{ref-bureauMetropolitanMicropolitanStatistical}{U.
C. Bureau n.d.})
...
\subsection{Initial Evidence: Spatial
Autocorrelation}\label{initial-evidence-spatial-autocorrelation}
...
\subsubsection{Global moran's I: Clustering of EV
...
Moran's I. The step we take here is to aggregate observed points into
...
clustering.(\citeproc{ref-rconsortiumAnalyzingGeospatialData2022}{R
Consortium 2022})

\paragraph{Moran's I using ZCTAs as bounding polygons and Queen
Definition of
...
\paragraph{Moran's I using Census Tracts as bounding polygons and Rook
definition of
...
\paragraph{visualize neiborhood
...
\subsubsection{Local Moran's I: detecting
...
can help use with a Moran's I statistic for each census
tract.(\citeproc{ref-geodasoftwareWeek6aLISA2021}{GeoDa Software 2021})
...
\paragraph{local moran permutation
...
Low-Low associations, 205 census tracts for High-High
associations.(\citeproc{ref-bivand2017package}{Bivand et al. 2017})
...
Local Moran's I, pseudo p-values from the conditional permutation test,
and identification of the association types for the significant census
tracts, take us to a more in-depth understanding of our EV point
pattern.

Insight into the hotspots and coldspots of EV adoption, corresponding to
High-High census tracts and Low-Low census tracts is especially helpful
because we are inclined to see the hotspots but ignore the coldspots.
The LISA Cluster Map reveals clusters centered census tracts with low EV
population themselves are also surrounded by neighbors with low EV
population. This finding is hardly possible to get if we only have the
positive cases, or already EV owning households to visualize. An
important aspect of interpreting positive spatial autocorrelation is
demonstrated here: we need to think of the bigger picture of clustering
as the contribution of attribute (in our case EV count) similarity and
locational similarity, thus, we won't ignore clusters that have similar
low EV counts also have a role to play in this picture.
(\citeproc{ref-LocalSpatialAutocorrelation}{Anselin 2020})
...
with the awareness of the underlying landscape of the state that can
have an effect on the adoption potential of different locations. We can
take a look at our EV population distribution together with the human
...
variations, and how population distribution as an explanation can have
...
\paragraph{\texorpdfstring{\texttt{zev\_ppp}}{zev\_ppp}}\label{zev_ppp}
...
\paragraph{\texorpdfstring{\texttt{pop\_int}}{pop\_int}}\label{pop_int}

\begin{itemize}
\tightlist
\item
  The data used for population counts by Census
  Tract(\citeproc{ref-Census2020DECENNIALDP2020.DP1}{U. S. C. Bureau,
  n.d.})
\end{itemize}
...
\paragraph{\texorpdfstring{\texttt{gpw\_ppp}}{gpw\_ppp}}\label{gpw_ppp}
...
Surface}\label{relative-intensity-surface}
...
of EV population is a special
one.\footnote{the interpretation of the ratio values have things to do
  with the different number of points between ppp objects, but the color
  variances is interpretable}(\citeproc{ref-moragaChapter21Intensity}{Moraga
n.d.})
...
\paragraph{divide regions for quadrat
...
\paragraph{computational approach: 999
...
\paragraph{calculate test statistic from
...
last section of the project, we are to think about where the EDA results
can take us.
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, bottomrule=.15mm, left=2mm, breakable, leftrule=.75mm, colback=white, opacityback=0, arc=.35mm, toprule=.15mm, colframe=quarto-callout-note-color-frame]
...
\tightlist
\item
  Data for locations of public charging stations are from AFDC
  api(\citeproc{ref-AllStationsAPI}{{``All {Stations API} {\textbar}
  {NREL}: {Developer Network}''} n.d.})
\end{itemize}
...
\bibitem[\citeproctext]{ref-AllStationsAPI}
{``All {Stations API} {\textbar} {NREL}: {Developer Network}.''} n.d.
...
\bibitem[\citeproctext]{ref-LocalSpatialAutocorrelation}
Anselin, Luc. 2020. {``Local {Spatial Autocorrelation} (1).''}
https://geodacenter.github.io/workbook/6a\_local\_auto/lab6a.html.

\bibitem[\citeproctext]{ref-bivand2017package}
Bivand, Roger, Micah Altman, Luc Anselin, Renato Assunção, Olaf Berke,
Andrew Bernat, and Guillaume Blanchet. 2017. {``Package {`Spdep'}.''}
\emph{Spatial Dependence: Weighting Schemes, Statistics, R Package
Version}, 1--1.

\bibitem[\citeproctext]{ref-Census2020DECENNIALDP2020.DP1}
Bureau, U. S. Census. n.d. {``PROFILE OF GENERAL POPULATION AND HOUSING
...
\bibitem[\citeproctext]{ref-geodasoftwareWeek6aLISA2021}
GeoDa Software. 2021. {``Week 6a: {LISA} and Local Moran ({Introduction}
to {Spatial Data Science}).''}

\bibitem[\citeproctext]{ref-Tl_2020_53_tract20}
{``Index of /Geo/Tiger/{TIGER2020PL}/{STATE}/53\_{WASHINGTON}/53.''}
n.d.b.
https://www2.census.gov/geo/tiger/TIGER2020PL/STATE/53\_WASHINGTON/53/tl\_2020\_53\_tract20.
Accessed November 17, 2024.

\bibitem[\citeproctext]{ref-Tl_2020_53_state20}
---------. n.d.a.
https://www2.census.gov/geo/tiger/TIGER2020PL/STATE/53\_WASHINGTON/53/.
Accessed November 29, 2024.

\bibitem[\citeproctext]{ref-Tl_2020_us_uac20}
{``Index of /Geo/Tiger/{TIGER2020}/{UAC}.''} n.d.
https://www2.census.gov/geo/tiger/TIGER2020/UAC/. Accessed November 29,
2024.

\bibitem[\citeproctext]{ref-Tl_2022_us_zcta520}
{``Index of /Geo/Tiger/{TIGER2022}/{ZCTA520}.''} n.d.
https://www2.census.gov/geo/tiger/TIGER2022/ZCTA520/. Accessed November
29, 2024.
...
\bibitem[\citeproctext]{ref-moragaChapter21Intensity}
Moraga, Paula. n.d. \emph{Chapter 21 {Intensity} Estimation {\textbar}
{Spatial Statistics} for {Data Science}: {Theory} and {Practice} with
{R}}. Accessed December 16, 2024.
...
\bibitem[\citeproctext]{ref-rconsortiumAnalyzingGeospatialData2022}
R Consortium. 2022. {``Analyzing {Geospatial Data} in {R} ({Sherrie
Xie}).''}
